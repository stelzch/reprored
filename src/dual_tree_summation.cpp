...

    {
        ...
        for (const auto op: operations) {
            if (op == OPERATION_REDUCE) {
                maximum_stack_size = std::max(maximum_stack_size, 2UL) - 2;
            } else {
                ++maximum_stack_size;
            }
        }
...
double DualTreeSummation::accumulate(void) {
#ifdef SCOREP
    // TODO: refactor function into methods and use autogenerated methods in lieu of ScoreP regions
    SCOREP_USER_REGION_DEFINE(region_local_reduce);
    ...
    SCOREP_USER_REGION_BEGIN(region_local_reduce, "local reduce", SCOREP_USER_REGION_TYPE_COMMON);
...
#ifdef SCOREP
    SCOREP_USER_REGION_END(region_local_reduce);
...
void DualTreeSummation::local_accumulate() {
    for (auto &[x, y]: local_compute_coords) {
        inbox[x] = local_accumulate(x, y);
    }
}
...
double DualTreeSummation::accumulate(uint64_t x, uint32_t y) {
    ...
    if (const auto e = inbox.find(x); e != inbox.end()) {
        return e->second;
    }
