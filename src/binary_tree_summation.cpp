...
BinaryTreeSummation::BinaryTreeSummation(uint64_t rank, const vector<region> regions, uint64_t k, MPI_Comm comm)
    :
      ...
      binary_tree(rank, chunked_array.get_k_chunks()),
      regions(regions),
      ...
      accumulation_buffer(k + chunked_array.get_local_size() + chunked_array.get_right_remainder()),
...
void BinaryTreeSummation::linear_sum_k() {
    ...
    if (chunked_array.get_right_remainder() > 0 && !chunked_array.has_left_neighbor_different_successor()) {
        // We do not reduce any summands on our own, we simply pass them to the successor
        assert(chunked_array.get_successor() >= 0);
        assert(chunked_array.get_local_size() == chunked_array.get_right_remainder());

        MPI_Isend(&accumulation_buffer[accumulation_buffer_offset_pre_k], chunked_array.get_local_size(), MPI_DOUBLE, chunked_array.get_successor(), MESSAGEBUFFER_MPI_TAG, comm, &send_req);
        return; // We are done here

    } else if (chunked_array.get_right_remainder() > 0 && !chunked_array.is_last_rank()) {
        // Sum & send right remainder
        assert(chunked_array.get_successor() >= 0);
        double acc = std::accumulate(&accumulation_buffer[accumulation_buffer_offset_pre_k + chunked_array.get_local_size() - chunked_array.get_right_remainder()], &accumulation_buffer[accumulation_buffer_offset_pre_k + chunked_array.get_local_size()], 0.0);
        MPI_Isend(&acc, 1, MPI_DOUBLE, chunked_array.get_successor(), MESSAGEBUFFER_MPI_TAG, comm, &send_req);
    }
    ...
    for (uint64_t i = chunked_array.get_left_remainder(); i + k - 1 < chunked_array.get_local_size(); i += k) {
    ...
    if (chunked_array.get_right_remainder() > 0 && chunked_array.is_last_rank()) {
        accumulation_buffer[accumulation_buffer_offset_post_k + target_idx++] = std::accumulate(
                                                            &accumulation_buffer[accumulation_buffer_offset_pre_k + chunked_array.get_local_size() - chunked_array.get_right_remainder()],
                                                            &accumulation_buffer[accumulation_buffer_offset_pre_k + chunked_array.get_local_size()],
                                                            0.0);
    }
    ...
    if (has_left_remainder) {
        ...
        left_remainder_accumulator = std::accumulate(&accumulation_buffer[accumulation_buffer_offset_pre_k],
                                                     &accumulation_buffer[accumulation_buffer_offset_pre_k + chunked_array.get_left_remainder()],
                                                     left_remainder_accumulator);
...
double BinaryTreeSummation::accumulate(void) {
    ...
    for (auto summand : binary_tree.get_rank_intersecting_summands()) {
        ...
        message_buffer.put(binary_tree.rankFromIndexMap(binary_tree.parent(summand)), summand, result);
    }
    message_buffer.flush();
    message_buffer.wait();

    double result = 0.0;
    const int root_rank = binary_tree.get_global_size() == 0 ? 0 : binary_tree.rankFromIndexMap(0);
    if (rank == root_rank) {
...
double BinaryTreeSummation::accumulate(const uint64_t index) {
    if (index & 1) {
        ...
        return accumulation_buffer[accumulation_buffer_offset_post_k + index - binary_tree.get_starting_index()];
    }

    const uint64_t maxX = (index == 0) ? binary_tree.get_global_size() - 1
        : min(binary_tree.get_global_size() - 1, index + binary_tree.subtree_size(index) - 1);
    const int maxY = (index == 0) ? ceil(log2(binary_tree.get_global_size())) : log2(binary_tree.subtree_size(index));

    const uint64_t largest_local_index = min(maxX, binary_tree.get_end_index() - 1);
    const uint64_t n_local_elements = largest_local_index + 1 - index;

    uint64_t elementsInBuffer = n_local_elements;

    double *destinationBuffer = static_cast<double *>(&accumulation_buffer[accumulation_buffer_offset_post_k + index - binary_tree.get_starting_index()]);
