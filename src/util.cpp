...
Distribution distribute_randomly(size_t const collection_size,
  ...
  std::generate(points.begin() + 1, points.end() - 1,
  ...
  // Shuffle to generate distributions where start indices are not monotonically
  // increasing
  ...
  assert(collection_size == std::reduce(shuffled_send_counts.begin(),
                                        shuffled_send_counts.end(), 0UL,
                                        std::plus<>()));
