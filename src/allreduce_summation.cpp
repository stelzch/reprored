...
AllreduceSummation::AllreduceSummation(MPI_Comm comm, size_t local_summands)
    :
        local_summands(local_summands),
        comm(comm),
        buffer(local_summands)
{
...
double *AllreduceSummation::getBuffer() {
    return buffer.data();
}

double AllreduceSummation::accumulate() {
    auto local_sum = std::reduce(buffer.begin(), buffer.end(), 0.0, std::plus<double>());
