...
MessageBuffer::MessageBuffer(MPI_Comm comm)
    : inbox(), targetRank(-1), awaitedNumbers(0), sentMessages(0),
      sentSummands(0), sendBufferClear(true), comm(comm) {
...
void MessageBuffer::flush() {
  ...
  assert(targetRank >= 0);
  MPI_Isend(static_cast<void *>(&outbox[0]), messageByteSize, MPI_BYTE,
            targetRank, MESSAGEBUFFER_MPI_TAG, comm, &reqs.back());
  sentMessages++;
...
const void MessageBuffer::receive(const int sourceRank) {
  ...
  MPI_Recv(static_cast<void *>(&buffer[0]),
           sizeof(MessageBufferEntry) * MAX_MESSAGE_LENGTH, MPI_BYTE,
...
void MessageBuffer::put(const int targetRank, const uint64_t index,
                        const double value) {
  if (outbox.size() >= MAX_MESSAGE_LENGTH || this->targetRank != targetRank) {
    flush();
  }

  /* Since we send asynchronously, we must check whether the buffer can
   * currently be written to */
  if (!sendBufferClear) {
    wait();
  }
  ...
  if (outbox.size() == MAX_MESSAGE_LENGTH)
    flush();
...
const double MessageBuffer::get(const int sourceRank, const uint64_t index) {
  ...
  // If not, we will wait for a message, but make sure no one is waiting for our
  // results.
  flush();
  wait();
  receive(sourceRank);
...
const void MessageBuffer::printStats() const {
  ...
  MPI_Reduce(localStats, globalStats, 3, MPI_LONG, MPI_SUM, 0, comm);
