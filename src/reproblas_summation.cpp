...
ReproblasSummation::ReproblasSummation(MPI_Comm comm, size_t local_summands, ReduceType type)
    : type(type), local_summands(local_summands), buffer(local_summands), comm(comm) {
...
double ReproblasSummation::accumulate() {
  ...
  if (type == ReduceType::REDUCE_BCAST) {
