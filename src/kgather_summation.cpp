...
KGatherSummation::KGatherSummation(uint64_t rank, const vector<region> regions, uint64_t k, MPI_Comm comm)
    :
      comm(comm),
      rank(rank),
      k(k),
...
vector<int> KGatherSummation::calc_send_counts(const vector<region>& regions) const {
...
double KGatherSummation::accumulate(void) {
    ...
    if(rank == 0) {
        result = std::accumulate(root_accumulation_buffer.begin(), root_accumulation_buffer.end(), 0.0);
...
const int KGatherSummation::get_rank() const {
    return rank;
}
